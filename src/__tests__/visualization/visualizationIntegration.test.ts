...
describe('Visualization System Integration', () => {
  ...
  describe('Dynamic HTML Page', () => {
    // Note: These tests require the Astro build to have run first
    // In CI/CD environment, the build runs after tests, so we skip these tests
    const htmlFileExists = fs.existsSync(dynamicHtmlPath);
    
    if (!htmlFileExists) {
      console.warn('⚠️  Skipping Dynamic HTML tests: machines-dynamic.html not found');
      console.warn('    This file is generated by the Astro build which runs after tests');
      return;
    }
